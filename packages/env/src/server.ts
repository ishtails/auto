...
export const env = createEnv({
	server: {
		...
		MOCK_LLM: z.enum(["true", "false"]).default("false").transform((val) => val === "true"),
		MOCK_RISK_AGENT: z.enum(["true", "false"]).default("false").transform((val) => val === "true"),
		MOCK_EXECUTION: z.enum(["true", "false"]).default("false").transform((val) => val === "true"),
