...
import { Droplet, ExternalLink, RefreshCw, Send, UserIcon } from "lucide-react";
...
import type { PrivyWalletLike } from "@/lib/privy-wallet-client";
...
export function UserDropdown() {
	...
	const operatorEns = useOperatorEnsProfile(user, wallets);
			...
			<DropdownMenu>
				...
				>
					...
					<DropdownMenuItem
						...
						onClick={() => {
							// Base UI Menu.Item uses onClick (not Radix-style onSelect).
							queueMicrotask(() => setSendDialogOpen(true));
						}}
					>
						<Send className="h-4 w-4 text-[#d97757]" />
						Send assets
					</DropdownMenuItem>
					<DropdownMenuSeparator className="bg-[#2a2a2a]" />
					<a
					...
					<DropdownMenuSeparator className="bg-[#2a2a2a]" />
					<DropdownMenuItem className="font-manrope text-[#dbc1b9] hover:bg-[#2a2a2a]">
						<button className="w-full text-left" onClick={logout} type="button">
							Log out
						</button>
					</DropdownMenuItem>
				</DropdownMenuContent>
